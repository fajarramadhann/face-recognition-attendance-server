...
def recognize_person(
    ...
    table_absen: str = MYSQL_ABSEN_TABLE) -> dict:
    """
    ...
    if get_person_resp["status"] == "success": # jika status nya sukses
        # insert record into mysql
        absen_data = {
            "person_id": person_id,
            "name": get_person_resp["person_data"]["name"],
            "jam_masuk": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        }
        
        with mysql_conn.cursor() as cursor:
            check_duplicate_data = f"SELECT * FROM {table_absen} WHERE (person_id= %s AND nama = %s) AND DATE(jam_masuk) = CURDATE()"
            cursor.execute(check_duplicate_data, (person_id, absen_data["name"]))
            duplicate_data = cursor.fetchone()
        
            if duplicate_data:
                update_absen = f"UPDATE {table_absen} SET jam_pulang = %s WHERE person_id = %s"
                values = (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), person_id)
                cursor.execute(update_absen, values)
                mysql_conn.commit()
                logger.info("Updated jam_pulang for person_id: %s in table_absen.✅️", person_id)   
            else:
                
                query = f"INSERT INTO {table_absen} (person_id, nama, jam_masuk) VALUES (%s, %s, %s)"
            
                try:
                    with mysql_conn.cursor() as cursor:
                        cursor.execute(query, (person_id, absen_data["name"], absen_data["jam_masuk"]))
                        mysql_conn.commit()
                        logger.info("Person with id: %s absen masuk to mysql db.✅️", person_id)
