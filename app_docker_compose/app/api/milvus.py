...
# config untuk milvus collection connection
def get_milvus_collec_conn( 
        collection_name: str,
        milvus_host: str = "0.0.0.0",
        milvus_port: int = 19530,
        vector_dim: int = 128, # dimensity
        metric_type: str = "L2", # euclidean distance
    ...
    if not utility.has_collection(collection_name): # jika collection_name tidak ada
        fields = [ # field yg bakal dipake buat di milvus
        ...
        schema = CollectionSchema( # schema collection yg akan dibuat
            fields=fields, description='face recognition system')
        milvus_collec_conn = Collection(name=collection_name, # bikin collection dengan name collection_name dan schema yg udh dibuat
